...
class CustomHtmlAppender<out O : Appendable>(
    val out: O,
    ...
    override fun onTagStart(tag: Tag) {
        ...
        if (tag.attributes.isNotEmpty()) {
            tag.attributesEntries.forEachIndexed { _, e ->
                ...
                out.append(e.key)
                out.append("=\"")
                // Prevent annoying escapes of script attributes.
                if (e.key.startsWith("on")) out.append(e.value) else out.escapeAppend(e.value)
                out.append('\"')
            }
        }
